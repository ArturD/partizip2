...
### Combined article charts

Article progress now has a latest-lesson chart and a daily chart, each combining all four cases with consistent colors, distinct line patterns, legends and expandable data tables. Lesson boundaries use a break of at least four hours within the selected difficulty, before tier filtering. The lesson x-axis counts selected answers across all cases; each line is smoothed independently over that case's answers. SMA and EMA support periods 1–100 (default 10), hide incomplete windows, and seed EMA with the first complete average. Each line displays up to 500 points after smoothing the full lesson. Daily lines show unsmoothed accuracy over 90 UTC days with gaps for no practice. Existing difficulty/tier filters apply throughout. No new migration is needed.
