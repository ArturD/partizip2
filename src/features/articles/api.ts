...
export async function articlesApi(request: Request, db: D1Database, learner: string) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/articles/progress') {
    ...
    const where = "learner_id = ? AND (? = '' OR tier = ?) AND (? = '' OR question = ?)";
    const bindings = [learner, tier, tier, question, question];
    const results = await db.batch([
      ...
      db.prepare(`SELECT substr(answered_at,1,10) day, COUNT(*) total, SUM(correct) correct FROM article_attempts WHERE ${where} AND answered_at >= ? GROUP BY day ORDER BY day`).bind(...bindings, new Date(Date.UTC(new Date().getUTCFullYear(), new Date().getUTCMonth(), new Date().getUTCDate() - 89)).toISOString()),
    ]);
  ...
  const previous = await db.prepare('SELECT noun_id, question FROM article_attempts WHERE learner_id = ? AND round_id = ?').bind(learner, body.roundId).all<{ noun_id: string; question: string }>();
  if (previous.results.some(row => row.noun_id !== noun.id) || steps.slice(0, steps.indexOf(question)).some(step => !previous.results.some(row => row.question === step))) return json({ error: 'Complete the earlier questions first.' }, 409);
  const result = solution(noun, question);
  await db.prepare('INSERT INTO article_attempts (id, learner_id, round_id, noun_id, content_version, tier, question, answer, expected, correct, answered_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING')
    .bind(body.id, learner, body.roundId, noun.id, noun.version, noun.tier, question, body.answer, result.expected, Number(body.answer === result.expected), new Date().toISOString()).run();
